...
// Версия: v.1.7.2 ||| URL: http://BlondeCMS.ru
...
if (isset($_POST['submit'])) {
    ...
    else $error .= '<li>Не введено имя</li>';

    if (isset($_POST["email"]) && !empty($_POST["email"])) $email = $_POST["email"];
    else $error .= '<li>Не введен E-mail</li>';

    if (isset($_POST["_tel"]) && !empty($_POST["_tel"])) $_tel = $_POST["_tel"];
// не проверяем телефон 		else $error .= '<li>Не введен номер телефона</li>';

    if (isset($_POST["text"]) && !empty($_POST["text"])) $text = $_POST["text"];
    else $error .= '<li>Не введён текст сообщения</li>';

    if (isset($_SESSION['captcha_keystring']) && $_SESSION['captcha_keystring'] == $_POST['keystring']) ;
    else $error .= '<li>Не верный код с рисунка</li>';

    if (strlen($text) > 3000) $error .= '<li>Максимальная длинна сообщения 2000 символов</li>';
    ...
    if (strlen($_tel) > 30) $error .= '<li>Максимальная длинна Телефона 30 символов</li>';
    if (strlen($_POST['keystring']) > 10) $error .= '<li>Максимальная длинна капчи 5 символов</li>';
    ...
    if (!empty($error)) echo('<ul style="margin:10px 20px; padding:10px; border-left:5px solid #f00; background:#fafafa">' . $error . '</ul>');
...
С уважением, Администратор <a href=\"http://$site\">$site</a><br> ");
                ...
                $mail->IsHTML(true);
                if (!$mail->AddEmbeddedImage('img/logo.jpg', 'my-attach', 'logo.jpg', 'base64', 'image/jpg'))
                    die ($mail->ErrorInfo);
...
?>
    ...
    <div id="forma">
        <form action='' method='POST'>
            ...
            <textarea name="text" maxlength="1500" required><?= $text; ?></textarea>

            <img style="float:left; margin:0 5px 0 0" src="./engine/cap.php?<?= session_name() ?>=<?= session_id() ?>">
            <input style="float:left; width:120px" type="text" name="keystring" placeholder="Код с картинки"
